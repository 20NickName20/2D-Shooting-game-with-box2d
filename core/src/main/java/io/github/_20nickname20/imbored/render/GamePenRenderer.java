...
public class GamePenRenderer extends Box2DDebugRenderer {

    public PenRenderer batch;
    ...
    public GamePenRenderer(ShaderProgram shader) {
        this(true, true, shader);
    }

    public GamePenRenderer(boolean drawBodies, boolean drawJoints, ShaderProgram shader) {
    ...
    protected void renderBody(Body body) {
        ...
        batch.draw(batch.penTexture, position.x, position.y, 40, 40);
    }
    ...
    private void drawSolidPolygon (int vertexCount, Color color, boolean closed) {
        renderer.setColor(color.r, color.g, color.b, color.a);
        lv.set(GamePenRenderer.vertices[0]);
        f.set(GamePenRenderer.vertices[0]);
        for (int i = 1; i < vertexCount; i++) {
            Vector2 v = GamePenRenderer.vertices[i];
            renderer.line(lv.x, lv.y, v.x, v.y);
            lv.set(v);
        }
